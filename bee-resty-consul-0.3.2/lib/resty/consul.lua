...
local function connect(self)
    ...
    if self.ssl then
        if DEBUG then
            ngx_log(ngx_DEBUG, "[Consul ssl] Handshaking, host: ", self.sni_host, " verify: ", self.ssl_verify)
        end
...
local function _request(self, method, path, args, body)
    ...
    if args.wait or args.index then
        ...
        if DEBUG then
            ngx_log(ngx_DEBUG, "[Consul] Blocking query timeout: ", timeout + (timeout / 16) + self.read_timeout)
        end
        ...
        for _, entry in ipairs(res.body) do
            if type(entry.Value) == "string" then
                ...
                if decoded ~= nil then
                    entry.Value = decoded
                    if DEBUG then
                        ngx_log(ngx_DEBUG, "[Consul] Decoded entry:\n", decoded)
                    end
                else
                    ngx_log(ngx_WARN, "[Consul] Could not decode Value")
                    if DEBUG then
                        ngx_log(ngx_DEBUG, entry.Value)
                    end
                end
            end
        end
        ...
        for _, el in ipairs(payload) do
            if el.KV then
                ...
                if val_type == "string" or val_type == "number" or val_type == "boolean" then
                    ...
                    if DEBUG then
                        ngx_log(ngx_DEBUG, "[Consul txn] Encoding value:\n", el.KV.Value)
                    end
        ...
        for _, entry in ipairs(res.body.Results) do
            if type(entry.KV.Value) == "string" then
                ...
                if decoded ~= nil then
                    ...
                    if DEBUG then
                        ngx_log(ngx_DEBUG, "[Consul txn] Decoded entry:\n", decoded)
                    end
                else
                    ngx_log(ngx_WARN, "[Consul txn] Could not decode Value")
                    if DEBUG then
                        ngx_log(ngx_DEBUG, entry.KV.Value)
                    end
                end
            end
        end
